	...
	void WriteTables(LOG log, IT::IdTable& idtable, LT::LexTable& lextable) {
		...
		*log.stream << "\nПозиция    Строка    Столбец    Лексема    ТИ";
		...
		for (int i = 0; i < idtable.size; i++) {
			...
			if (idtable.table[i].iddatatype == IT::DT_INT) {
				strcpy_s(iddatatype, "целочисленный");
			}
			else if (idtable.table[i].iddatatype == IT::DT_STR) {
				strcpy_s(iddatatype, "строковый");
			}
			else if (idtable.table[i].iddatatype == IT::DT_BOOL) {
				strcpy_s(iddatatype, "логический");
			}
			else if (idtable.table[i].iddatatype == IT::DT_CHAR) {
				strcpy_s(iddatatype, "символьный");
			}
			else {
				strcpy_s(iddatatype, "неизвестно");
			}
